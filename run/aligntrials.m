...
fname = strcat('.\img_trial\',A(3).name);
        info = imfinfo(fname);
...
for f=3:length(A) %1~3ok
    ...
    for zz = 1:z_slice_num
        fname = strcat('.\img_trial\',A(f).name);
        info = imfinfo(fname);
...
%     imwrite(uint16(movingRegistered),strcat('result_',sprintf('%03d',f),'_registered_rigid.tif'));
    ...
    save(strcat('tform_trial_',num2str(f),'.mat'),'tform','movingRegistered');
end
writerObj = VideoWriter('AlignTrials.avi');
